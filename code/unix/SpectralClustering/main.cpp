...
CImg<> GetAffinityMatrix(CImg<> points, float sigma, unsigned k) {
	...
	float coef = 2*sigma*sigma;		// coef a base de variance
	CImg<> result = new CImg<>(numberOfPoints,numberOfPoints,points.depth(),1,0.0);	// matrice d'affinite

	cimg_forX(result, i) {
		for (int j = i+1; j < points.width();++j) {
			// remplissage de la partie superieure droite
			result(i,j) = expf(-Square(
								GetNorme(	points(i/points.width(),i%points.width(), 0),
...
CImg<> GetDSqrtInvMatrix(CImg<> & affinity) {
	CImg<> res = new CImg<>(affinity.width(),affinity.height(),1,1,0.0);	// matrice L
...
CImg<> GetLMatrix(CImg<> & affinity) {
	CImg<> dsi = GetSqrtInvMatrix(affinity);
